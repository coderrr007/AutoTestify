...
PYTHON_LANGUAGE = Language(
    os.path.join(settings.BASE_DIR, "tree-sitter-python/libtree-sitter-python.so"),
    "python",
)
...
class Neo4jGraph:
    ...
    def create_function_node(self, function_name, file_name, code):
        with self.driver.session() as session:
            session.write_transaction(
                self._create_function_node, function_name, file_name, code
            )
    ...
    def create_import_relationship(self, function_name, import_name):
        with self.driver.session() as session:
            session.write_transaction(
                self._create_relationship, function_name, import_name
            )

    @staticmethod
    def _create_function_node(tx, function_name, file_name, code):
        tx.run(
            "MERGE (n:Node {name: $name, file: $file, code: $code})",
            name=function_name,
            file=file_name,
            code=code,
        )
    ...
    def create_test_case_prompt(
        self, function_name, file_name, related_data, code, context
    ):
    ...
    def generate_test_cases(
        self, openai_type, function_name, file_name, code, similar_functions
    ):
        ...
        if related_data:
            messages = self.create_test_case_prompt(
                function_name, file_name, related_data[0], code, context_set
            )
            if openai_type == "azure":
                response = openai.ChatCompletion.create(
                    engine="gpt35turbo", messages=messages, temperature=0.5
                )
            else:
                response = openai.ChatCompletion.create(
    ...
    def _query_related_data(tx, function_name):
        ...
        for record in result:
            ...
            data.append({"function": record["function"], "imports": imports})
            ...
            for file in files:
                if file.endswith(".py") and file not in [
                    ...
                    for func_name, _, _, _ in functions:
                        for imp in imports:
                            graph.create_import_relationship(
                                func_name.strip(), imp.strip()
                            )
        ...
        for function in all_functions:
            ...
            if function.get("file_name"):
                ...
                similar_functions = find_similar_functions(
                    index, function_ids, embedding, 2
                )
                test_cases = graph.generate_test_cases(
                    openai_type,
                    function_name,
                    file_name,
                    function["code"],
